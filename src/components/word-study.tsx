...
export function WordStudy({
    ...
    const { hidden, toggleHidden } = useCardSettings();
    const hideWord = !preview && hidden.word;
    const hideReading = !preview && hidden.reading;
    const hideMeaning = !preview && hidden.meaning;
    const {
        saved,
        ready,
        busy,
    ...
    const pointer = useRef<{ id: number; x: number; width: number } | null>(
        null,
    );
    ...
    function moveSavedWord(direction: number) {
        ...
        setIndex(
            Math.max(
                0,
                Math.min(savedIndex + direction, savedWords.length - 1),
            ),
        );
    }
    async function removeSavedWord() {
        if (!word || !ready || busy || deciding.current) return;
        deciding.current = true;
        resetDrag();
        try {
            if (await updateSaved(word.id, true)) {
                setIndex(
                    Math.min(savedIndex, Math.max(0, savedWords.length - 2)),
                );
                setNotice(`${word.text}을(를) 단어장에서 삭제했어요.`);
            }
        } finally {
    ...
    function finishDrag(event: PointerEvent<HTMLDivElement>) {
        ...
        if (savedOnly && action) moveSavedWord(action === "known" ? 1 : -1);
    ...
    return (
        <>
            {!preview && (
                <>
                    <p className="eyebrow">
                        {savedOnly ? "MY WORDS" : "DAILY PRACTICE"}
                    </p>
            ...
            {!preview && !savedOnly && (
                <Link className="secondary-button mt-6" href="/learn">
                    ← Day 목록으로
                </Link>
            )}
            ...
            {savedOnly && account && (
                <button
                    ...
                    onClick={async () => {
                        setNotice("");
                        if (await importLocal())
                            setNotice(
                                "이 브라우저의 단어를 계정 단어장에 추가했어요. 기존 브라우저 단어장도 유지돼요.",
                            );
                    }}
                >
                    이 브라우저의 단어 가져오기
            ...
            {savedOnly && (!ready || !word) ? (
                ...
                <section className="panel mx-auto mt-10 max-w-2xl text-center">
                    ...
                    <p className="mt-4 text-muted">
                        외우고 싶은 단어는 내 단어장에서 다시 볼 수 있어요.
                    </p>
                    <div className="mt-6 flex flex-wrap justify-center gap-3">
                        {dayIndex < days.length - 1 && (
                            <Link
                                className="primary-button"
                                href={`/learn/${days[dayIndex + 1].id}`}
                            >
                                다음 Day 학습하기 →
                            </Link>
                        )}
                ...
                >
                    ...
                    <div
                        ...
                        onPointerMove={(event) => {
                            ...
                            if (start?.id === event.pointerId)
                                setDrag({
                                    distance: event.clientX - start.x,
                                    width: start.width,
                                });
                        }}
                    ...
                    >
                        <p
                            className="text-sm text-card-muted"
                            aria-live="polite"
                            aria-atomic="true"
                        >
                            {preview
                                ? "미리보기 · 1 / 1"
                                : savedOnly
                                  ? `내 단어장 · ${savedIndex + 1} / ${savedWords.length}`
                                  : `${day.label} · ${index + 1} / ${day.words.length}`}
                        </p>
                        {!preview && (
                            <details
                                className="mt-4 rounded-xl border border-white/20 bg-white/5 text-left"
                                onPointerDown={(event) =>
                                    event.stopPropagation()
                                }
                            >
                                <summary className="min-h-11 cursor-pointer rounded-xl px-4 py-3 text-sm font-semibold">
                                    카드 표시 설정
                                </summary>
                                <fieldset className="flex flex-wrap gap-x-4 gap-y-1 border-t border-white/20 px-4 py-2">
                        ...
                        <SpeechButton
                            text={word.reading}
                            label="단어 듣기"
                            hideTextInLabel={hideReading || hideWord}
                        />
                        ...
                        {word.example && (
                            <div className="border-t border-white/20 pt-6">
                                <p className="mb-3 text-xs font-semibold text-card-muted">
                                    예문
                                </p>
                                <p
                                    lang={hideWord ? undefined : "ja"}
                        ...
                        {savedOnly ? (
                            <>
                                <button
                                    className="secondary-button"
                                    disabled={
                                        !ready || busy || savedIndex === 0
                                    }
                                    onClick={() => moveSavedWord(-1)}
                                >
                                    ← 이전 단어
                                </button>
                                <button
                                ...
                                <button
                                    ...
                                    disabled={
                                        !ready ||
                                        busy ||
                                        savedIndex === savedWords.length - 1
                                    }
                                    onClick={() => moveSavedWord(1)}
                                >
                                    다음 단어 →
